...
for i, ax in enumerate(axs):
    ...
    if i > 3:
        for brln in brlns:
            ...
            #print(brln)
            #print(numpy.round(xdf.AVG_EN.values, 0))
    ...
    if i < 2:
        for brln in brlns:
            ...
            ax.set_ylim(0, 105)
            if i == 0:
                ax.legend(frameon=False, ncol=1, fontsize=9, loc="lower right")
            else:
                ax.legend(frameon=False, ncol=1, fontsize=9, loc="lower right")
    if (i > 1) and (i < 4):
        for brln in brlns:
            xdf = df[df["BRLN_TRUE"] == brln]
            print(brln)
            #print(xdf.AVG_PERR_MLE_RI.values)
            #print(xdf.AVG_AERR_MLE_RI.values)
            #print(xdf.STD_AERR_MLE_RI.values)
            #print(xdf.STD_AERR_MLE_RI.values)
            #print(xdf.AVG_BRLN_MLE_RI.values)
            #print(xdf.STD_BRLN_MLE_RI.values)
            #print(xdf.AVG_Q1.values)
            #print(xdf.STD_Q1.values)
            print(xdf.AVG_EN.values)
            print(xdf.STD_EN.values)
    ...
    ax0.text(85, 105, r"A)", fontsize=12)
    ax1.text(85, 105, r"D)", fontsize=12)
    ax2.text(85, 105, r"B)", fontsize=12)
    ax3.text(85, 105, r"E)", fontsize=12)
    ax4.text(85, 20000, r"C)", fontsize=12)
    ax5.text(85, 20000, r"F)", fontsize=12)
